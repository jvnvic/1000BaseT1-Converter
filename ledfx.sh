...
get_latest_ledfx_tag () {
  # asks GitHub for the latest release tag
  python3 - <<'PY'
import json, urllib.request
url = "https://api.github.com/repos/LedFx/LedFx/releases/latest"
...
install-ledfx () {
  ...
  latest_tag=$(get_latest_ledfx_tag)
  echo "Latest tag is: $latest_tag"
  "$HOME/.ledfx/bin/python" -m pip install "git+https://github.com/LedFx/LedFx.git@$latest_tag"
...
update-ledfx () {
  ...
  latest_tag=$(get_latest_ledfx_tag)
  echo "Updating LedFx to $latest_tag from GitHub..."
  "$HOME/.ledfx/bin/python" -m pip install --upgrade --force-reinstall --no-deps --no-cache-dir "git+https://github.com/LedFx/LedFx.git@$latest_tag"
